...
export class Engineer extends Creep {
    ...
    static dispatch(room: Room) {
        ...
        let engineers = room.localCreeps(Role.ENGINEER)
    ...
    static shouldSpawn(room: Room): boolean {
        ...
        if (room.constructionSites.length < 1 && room.localCreeps(Role.ENGINEER).length > 1) { return false }
        if (room.localCreeps(Role.ENGINEER).length >= room.controller.level) { return false }
        return true
    }
